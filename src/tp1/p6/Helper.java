...
public class Helper {
 ...
 // VALIDAR DOUBLE
 	public static Double validar_Double(Scanner ingreso, String inputMessage) {
 		Double doubleValue = 0.0;
 		String valorIngresado;
 		while (true) {
 			try {
 				System.out.print(inputMessage);
 				valorIngresado = ingreso.nextLine();
 				doubleValue = Double.parseDouble(valorIngresado);
 				return doubleValue;
 			} catch (Exception exception) {
 	...
 	public static Double validar_DoublePositivo(String mensaje) {
 		Scanner ingreso = new Scanner(System.in); 
 		double doublePositivo;
 		while (true) {
 			doublePositivo = validar_Double(ingreso, mensaje);
 			if (doublePositivo <= 0) {
 				System.out.println("No puede ingresar numeros negativos o 0!!!");
 			} else {
 				break;
 			}
 		}
 		return doublePositivo;
 	}
